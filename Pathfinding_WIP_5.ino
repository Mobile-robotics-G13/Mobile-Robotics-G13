...
// ================= ROUTE STATE (MEMORY) =================
String previousNode = "0";   // Starting at node 0
String currentNode = "0";    // Currently at node 0
String targetNode = "4";     // Pointing towards node 4
...
void setup() {
  ...
  // UPDATED: Start at node 0, pointing towards node 4
  previousNode = "0";  // We're starting at 0
  currentNode = "0";   // Currently at 0
  targetNode = "4";    // Pointing towards 4
  
  sendPosition(currentNode);
  Serial.println("Starting at Node 0, pointing towards Node 4");
...
void handleIntersection() {
  // Debug print showing Previous -> Current
  ...
  while (targetNode == currentNode && remainingRoute.length() > 0) {
    Serial.print("Skipping redundant node: ");
  ...
  // 4. CHECK IF STRAIGHT PATH
  if (isPathStraight(currentNode, targetNode)) {
    Serial.println(">>> CONTINUING STRAIGHT");
    moveForward(maxSpeed, maxSpeed);
    delay(150);
    
    // Update memory for straight path
    previousNode = currentNode;
    currentNode = targetNode;
    return;
  }

  // 5. EXECUTE TURN (now using previousNode for context)
  decideTurn(previousNode, currentNode, targetNode);

  // 6. Update Memory
  previousNode = currentNode;  // Where we were becomes previous
  currentNode = targetNode;    // Where we're going becomes current
  
  Serial.print("Memory updated -> Previous: ");
  Serial.print(previousNode);
...
void decideTurn(String from, String current, String to) {
  ...
  if (current == "0") {
    if (to == "6") {
      // Going to node 6 from node 0
      if (from == "4" || from == "0") {
        // Coming from 4 -> at 0 -> going to 6 = turn right
        // OR starting at 0 pointing towards 4, then going to 6 = turn right
        isLeft = false;
        foundMatch = true;
      } else if (from == "-1") {
        // Starting position going to 6 = turn right
        isLeft = false;
        foundMatch = true;
      }
    } else if (to == "4") {
      // Going to node 4 from node 0
      if (from == "6") {
        // Coming from 6 -> at 0 -> going to 4 = turn left
        isLeft = true;
        foundMatch = true;
      } else if (from == "0") {
        // Starting at 0 pointing to 4, and actually going to 4 = go straight (handled by isPathStraight)
  ...
  // ====================
    ...
    if (to == "6") {
      // At node 2 going to 6
      if (from == "3") {
        // Coming from 3 -> at 2 -> going to 6 = turn right
        isLeft = false;
        foundMatch = true;
      }
    } else if (to == "3") {
      // At node 2 going to 3
      if (from == "6") {
        // Coming from 6 -> at 2 -> going to 3 = turn left
        isLeft = true;
        foundMatch = true;
      }
    }
  ...
  // ====================
    ...
    if (to == "7") {
      // At node 3 going to 7
      if (from == "2") {
        // Coming from 2 -> at 3 -> going to 7 = turn right
        isLeft = false;
        foundMatch = true;
      }
    } else if (to == "2") {
      // At node 3 going to 2
      if (from == "7") {
        // Coming from 7 -> at 3 -> going to 2 = turn left
        isLeft = true;
        foundMatch = true;
      }
    }
  ...
  // ====================
    ...
    if (to == "7") {
      // At node 4 going to 7
      if (from == "0") {
        // Coming from 0 -> at 4 -> going to 7 = turn right
        isLeft = false;
        foundMatch = true;
      }
    } else if (to == "0") {
      // At node 4 going to 0 (U-turn or straight depending on layout)
      if (from == "7") {
        // Coming from 7 -> at 4 -> going to 0 = turn left
        isLeft = true;
        foundMatch = true;
      }
    }
  ...
  // ====================
  // NODE 6 LOGIC (Right Junction)
  // ====================
  else if (current == "6") {
    if (to == "0") {
      // At node 6 going to 0
      if (from == "2") {
        // Coming from 2 -> at 6 -> going to 0 = turn left
        isLeft = true;
        foundMatch = true;
      } else if (from == "1") {
        // Coming from 1 -> at 6 -> going to 0 = turn right
        isLeft = false;
        foundMatch = true;
      }
    } else if (to == "1") {
      // At node 6 going to 1
      if (from == "0") {
        // Coming from 0 -> at 6 -> going to 1 = turn left
        isLeft = true;
        foundMatch = true;
      } else if (from == "2") {
        // Coming from 2 -> at 6 -> going to 1 = turn right
        isLeft = false;
        foundMatch = true;
      }
    } else if (to == "2") {
      // At node 6 going to 2
      if (from == "0") {
        // Coming from 0 -> at 6 -> going to 2 = turn right
        isLeft = false;
        foundMatch = true;
      } else if (from == "1") {
        // Coming from 1 -> at 6 -> going to 2 = turn left
        isLeft = true;
        foundMatch = true;
      }
    }
  ...
  // ====================
  // NODE 7 LOGIC (Left Junction)
  // ====================
  else if (current == "7") {
    if (to == "1") {
      // At node 7 going to 1
      if (from == "5") {
        // Coming from 5 -> at 7 -> going to 1 = turn left
        isLeft = true;
        foundMatch = true;
      } else if (from == "3" || from == "4") {
        // Coming from 3 or 4 -> at 7 -> going to 1 = turn right
        isLeft = false;
        foundMatch = true;
      }
    } else if (to == "3") {
      // At node 7 going to 3
      if (from == "5") {
        // Coming from 5 -> at 7 -> going to 3 = turn right
        isLeft = false;
        foundMatch = true;
      } else if (from == "4") {
        // Coming from 4 -> at 7 -> going to 3 = turn left
        isLeft = true;
        foundMatch = true;
      } else if (from == "1") {
        // Coming from 1 -> at 7 -> going to 3 = turn left
        isLeft = true;
        foundMatch = true;
      }
    } else if (to == "4") {
      // At node 7 going to 4
      if (from == "5") {
        // Coming from 5 -> at 7 -> going to 4 = turn left
        isLeft = true;
        foundMatch = true;
      } else if (from == "3") {
        // Coming from 3 -> at 7 -> going to 4 = turn right
        isLeft = false;
        foundMatch = true;
      } else if (from == "1") {
        // Coming from 1 -> at 7 -> going to 4 = turn right
        isLeft = false;
        foundMatch = true;
      }
    } else if (to == "5") {
      // At node 7 going to 5
      if (from == "1") {
        // Coming from 1 -> at 7 -> going to 5 = 180 turn
        Serial.println("Action: 180 Turn to Node 5");
        perform180Turn();
        return;
      } else if (from == "3" || from == "4") {
        // Coming from 3 or 4 -> at 7 -> going to 5 = turn left/right
        isLeft = (from == "3");  // Left if from 3, right if from 4
        foundMatch = true;
      }
    }
  }
  ...
  if (foundMatch) {
    ...
    Serial.println("WARNING: No turn logic matched! Attempting default turn...");
